...
function FlashcardStudy() {
  ...
  const fetchStudyData = async () => {
    ...
    try {
      ...
      if (!deckResponse.ok) {
        throw new Error(`Erro ao buscar informações do baralho: ${deckResponse.statusText}`);
      }
      ...
      if (cardsData.cards && cardsData.cards.length > 0) {
        setCurrentFlashcard(cardsData.cards[0]); 
      } else {
  ...
  const handleDifficultyClick = async (difficulty) => {
    ...
    const rating = difficultyMap[difficulty];
    if (!rating) return;
      ...
      <main className="flashcard-study-main-content">
        ...
        {!showBack ? (
          ...
          <div className="difficulty-buttons">
            <button className="btn-difficulty btn-wrong" onClick={() => handleDifficultyClick('again')}>Errado</button>
            <button className="btn-difficulty btn-difficult" onClick={() => handleDifficultyClick('hard')}>Difícil</button>
